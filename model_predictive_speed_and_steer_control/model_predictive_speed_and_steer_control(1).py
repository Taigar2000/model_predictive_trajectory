...
TARGET_SPEED = 12.6 / 3.6  # [m/s] target speed
...
MAX_SPEED = 12.6 / 3.6  # maximum speed [m/s]
...
def linear_mpc_control(xref, xbar, x0, dref):
    ...
    constraints += [x[:, 0] == x0]
    #constraints += [[ math.sin(t+0.5*math.pi)+10 for t in np.arange(0., 2*math.pi, 0.01)]]
    #constraints += [(x[0, :]-5)**2+(x[1, :]-0.1)**2<=0.1]
    #constraints += [((2-x[0,:])*(2-x[0,:])+x[1,:]*x[1,:])<=2]
...
def get_straight_course2(dl):
    ...
    ay = [0.0, -1.0, 10.0, 0.0, -1.0, 10.0, 0.0]
    cx, cy, cyaw, ck, s = cubic_spline_planner.calc_spline_course(
        ax, ay, ds=dl)
...
def main():
    ...
    #cx, cy, cyaw, ck = get_straight_coursek(dl)
